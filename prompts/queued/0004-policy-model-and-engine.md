# 0004 — Policy model & decision engine
...
- `docs/learnings/` — read summaries; open `0002` (the contract's output shapes
  ...
  (the bundle table).
- In the **bastion repository**, `docs/PLAN.md`: **D5a** (the three policy axes),
  **D6a** (credential methods), **D11** (hop direction), **D12** (the three
  filtering tiers). This engine's output vocabulary is exactly what the bastion
...
entry bastion; target hostname, labels, and zone; and live grants.

**What a rule emits** (PLAN §5.2) — the snapshot, whose vocabulary is the
bastion's enforcement surface:
...
- **filter policy**: an ordered rule list **or** a restricted-exec allow-list,
  never both (bastion D12);
- **target credential method** + parameters (bastion D6a);
...
- Time is an **input**, never `time.Now()` inside the engine — otherwise
  simulation over historical traffic (0011) is impossible and tests are flaky.
- Produces `(snapshot, explanation)` where the explanation names the matched
  ...
  the feature 0011 exposes and the bastion's disclosure rule depends on.
...
## Out of scope
- Storing bundles or decisions (0003 has the tables; 0007 writes decisions).
...
- Simulation and the authoring API (0011) — but keep evaluation pure so both are
...
`docs/learnings/0004-policy-model-and-engine-learnings.md`. Summary block MUST
...
the measured evaluation budget. Phases 0007, 0011 and 0012 all build directly on
