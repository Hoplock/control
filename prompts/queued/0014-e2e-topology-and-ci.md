# 0014 — Cross-repo E2E topology, CI gate & hardening
...
- `docs/learnings/` — read every summary; open the ones whose setup you must
  wire (esp. `0002` conformance, `0005` fleet enrollment, `0007` authorize,
  `0008` events, `0009` audit assertions, `0012` grants).
- In the **bastion repository**: its `deploy/` topology and its own e2e phase.
  You are extending that idea across the repository boundary, not rebuilding it.

## Objective
Prove the thing neither repository can prove alone: a **real bastion** enforcing
decisions from a **real PDP**. The bastion's own e2e proves it enforces what a
...
2. **management** — `cmd/management`, both listeners, seeded with a policy
   bundle, targets and labels, identities, and fleet enrollment.
3. **bastion** — built from the **bastion repository**, configured to point at
   this server. Pin the bastion revision explicitly and record it, so a failure
   is attributable to a change on one side or the other.
4. **bastion-relay** — a second bastion in a "protected zone", reachable **only**
   by its own outbound relay registration (bastion D11). Enforce the isolation in
...
- **Outage is not a deny**: stop this server; a connecting user is told it is an
  outage and given a session id, not "access denied" (M11, bastion §4.3).
...
- **Cache + revocation interaction**: with the event stream unhealthy, this
  server issues no hint and the bastion re-authorizes every connection.
...
- **`govulncheck ./...`** failing the build on any finding, with the same
  reasoning the bastion repo records: this server holds credentials, terminates
...
- Keep `make conform` in CI against both this server and the bastion's mock.
...
- Performance tuning beyond the budgets already stated (M5, 0007).
...
- The pinned bastion revision is recorded, and the docs say how to move it.

## Definition of Done & hand-off
Per `docs/PROTOCOL.md`. Move to `implemented/`; add
`docs/learnings/0014-e2e-topology-and-ci-learnings.md`. Summary block MUST
document how to run the topology locally, the seed/fixture layout, each scenario
and what it proves, how the bastion revision is pinned and bumped, how the
